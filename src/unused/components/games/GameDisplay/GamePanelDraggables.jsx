...
import Connection from "../../../../components/widgets/Connection";
...
function GamePanelDraggables(props) {
    ...
    const renderDraggables = () => {
        ...
        for (const key in rooms) {
            ...
            let gamepanel = findGamePanelByRoom(rooms[key].room.room_slug);

            if (!gamepanel) continue;
            //let's not show gamepanels that are available to reserve
            if (gamepanel.available) continue;

            draggables.push(
                <GamePanelDraggable
                    key={"gpdraggable-" + gamepanel.id}
                    gamepanel={gamepanel}
                />
            );
        }
    ...
    return (
        <>
            <Box
                ...
                right="0rem"
                zIndex="1000"
                display={layoutMode == "off" ? "block" : "none"}
            >
                <ActionMenu />
            </Box>
            {/* <LoadingBox /> */}
...
function GamePanelDraggable(props) {
    ...
    if (
        !gamepanel.isPrimary &&
        (gamepanel.canvasRef || gamepanel?.room?.isReplay)
    ) {
        return <></>;
    ...
    return (
        ...
        >
            <Box
                ...
                transition={
                    isPrimary ? "" : "width 0.1s ease, height 0.1s ease"
                }
