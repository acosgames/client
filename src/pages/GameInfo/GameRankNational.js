...
export default function GameRankNational({ countrycode }) {

    countrycode = countrycode || 'US';

    let [rankings] = fs.useWatch('rankings/national/' + countrycode);
    let [rankingsCount] = fs.useWatch('rankings/national/' + countrycode + '/count');
    ...
    useEffect(() => {
        let g = fs.get('game');
        if (g)
            findGameRankNational(g.game_slug);
    }, [game?.game_slug])
        ...
        <Box w="100%" maxW={["100%", "100%", "100%", "95%", "70%", "60%"]} pt="0" pb="10rem" px={["1rem", "3rem"]}>
            <VStack w="100%" spacing="1rem" alignItems={'flex-start'}>
                <VStack
                    ...
                    alignItems={'center'}
                >
                    <Heading as="h2" color="gray.0" fontSize={['2.4rem', '2.4rem', "3rem"]} fontWeight={'600'}>National Rankings</Heading>
                </VStack>
...
function PlayerRanking({ displayname, rank, countrycode, isLocalPlayer, portraitid, ratingFormatted, rating }) {
    return (
        <HStack
            position="relative"
            width="100%"
