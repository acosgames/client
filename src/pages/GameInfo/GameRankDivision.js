...
export default function GameRankDivision({ }) {

    let [rankings] = fs.useWatch('rankings/division');
    let [rankingsCount] = fs.useWatch('rankings/division/count');
        ...
        <Box w="100%" maxW={["100%", "100%", "100%", "95%", "70%", "60%"]} pt="0" pb="10rem" px={["1rem", "3rem"]}>
            <VStack w="100%" spacing="1rem" alignItems={'flex-start'}>
                <VStack
                    ...
                    alignItems={'center'}
                >
                    <Heading as="h2" color="gray.0" fontSize={['2.4rem', '2.4rem', "3rem"]} fontWeight={'600'}>Division Rankings</Heading>
                    <Heading as="h3" color="gray.20" fontSize={['1.6rem', '1.6rem', "2rem"]} fontWeight={'500'}>{division_name || 'Unknown Division'}</Heading>
                </VStack>
...
function PlayerRanking({ displayname, rank, flagCode, isLocalPlayer, portraitid, ratingFormatted, rating }) {
    return (
        <HStack
            position="relative"
            width="100%"
