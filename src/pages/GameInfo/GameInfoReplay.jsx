...
function GameInfoReplayContent({ game_slug }) {
    ...
    let { bgWidth, bgHeight } = calculateGameSize(
        w,
        h,
        replaySettings.resow,
        replaySettings.resoh,
        1
    );
...
function PlayersList({ room_slug }) {
    ...
    if (room.maxteams == 0) return <PlayersFFA gamepanelid={gamepanel.id} />;
    ...
    return (
        <Box w="100%" h="3rem">
            <HStack h="3rem" spacing="0" justifyContent={"center"}>
                ...
                <ReplayPosition gamepanel={gamepanel} />
                <Button
                    p="0"
                    m="0"
                    onClick={() => {
...
function ReplayPosition({ gamepanel }) {
    let replayIndex = useBucketSelector(
        btGamePanels,
        (bucket) => bucket[gamepanel?.id || 0]?.room?.replayIndex
    );
