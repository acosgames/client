...
function Screen1({}) {
  ...
  let gamestatus = useBucket(btGameStatus, (bucket) => bucket[primaryId]);
  let roomstate = useBucket(btPrimaryState, (bucket) => bucket.roomstate);
...
function GameFinish({}) {
  let players = useBucket(btPrimaryState, (bucket) => bucket.players);
  let teams = useBucket(btPrimaryState, (bucket) => bucket.teams);
  ...
  if (
    ...
    if (
      ...
      (!teams && playerRanks.rankOne.find((r) => r == localPlayer.shortid))
    )
      title = "Victory";
    else title = "Defeat";
  }
